...
class GuiLogHandler(logging.Handler):
    """
    把日志转发到 Tk Text 控件。
    注意：不直接持有 widget（避免循环引用），用 weakref + `after(0)`。
    """

    def __init__(self, get_app_callable: Callable[[], Any]):
        super().__init__(level=logging.DEBUG)
        try:
            self._get_app = weakref.ref(get_app_callable)
            self._use_weakref = True
        except TypeError:
    ...
    def _resolve_app(self) -> Any:
        ...
        return self._get_app() if callable(self._get_app) else self._get_app
